...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_record' && $is_admin) {
    ...
    // Update Unit/Office and Age in users table
    $new_unit = trim($_POST['unit_office'] ?? '');
    $new_age = isset($_POST['age']) ? (int)$_POST['age'] : null;

    if ($new_unit !== '' || $new_age !== null) {
        $update_parts = [];
        $update_params = [];
        
        if ($new_unit !== '') {
            $update_parts[] = "unit = ?";
            $update_params[] = $new_unit;
        }
        
        if ($new_age !== null) {
            $update_parts[] = "age = ?";
            $update_params[] = $new_age;
            
            // Calculate default birthday (Jan 1 of the calculated year)
            $currentYear = (int)date('Y');
            $birthYear = $currentYear - $new_age;
            $birthday = "$birthYear-01-01";
            $update_parts[] = "birthday = ?";
            $update_params[] = $birthday;
        }
        
        if (!empty($update_parts)) {
            $update_params[] = $user_id_to_save;
            $sql_user = "UPDATE users SET " . implode(", ", $update_parts) . " WHERE id = ?";
            $stmt_user = $pdo->prepare($sql_user);
            $stmt_user->execute($update_params);
        }
    }
...
<!-- Editor Header Section -->
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Synchronize Age changes with calculations
            const inputAge = document.getElementById('input-age');
            if (inputAge) {
                inputAge.addEventListener('input', calculateHealthMetrics);
            }
